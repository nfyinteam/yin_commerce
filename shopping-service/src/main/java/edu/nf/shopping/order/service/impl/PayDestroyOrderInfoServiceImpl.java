...
public class PayDestroyOrderInfoServiceImpl implements DestroyOrderInfoService {
     ...
     */
    @Override
    @CacheEvict(value = "orderCache", key = "#orderInfo.orderId", condition = "#orderInfo.orderState == '待付款'")
    public void destroyOrder(OrderInfo orderInfo) {
        try {
            OrderInfo o = orderDao.getOrderInfoByOrderId(orderInfo.getOrderId());
            if(o != null){
                if(o.getOrderState().equals("待付款")){
                    //修改状态
                    o.setOrderState("已失效");
                    //恢复库存
     ...
     */
    @RabbitListener(queues = {OrderRabbitConfig.ORDER_PAY_DESTROY_QUEUE, })
